...
export default function SubmitTab({ detail, onSubmitDone }: Props) {
  ...
  const { addSubmission } = useStoreContext();
  ...
  const validate = (): boolean => {
    ...
    if (!teamName.trim()) newErrors.teamName = '팀 이름을 입력해주세요.';
    if (isMultiStep) {
      steps.forEach(item => {
        if (!values[item.key]?.trim()) {
          newErrors[item.key] = '필수 항목을 입력해주세요.';
        } else if ((item.format === 'url' || item.format === 'pdf_url') && values[item.key].trim()) {
  ...
  const handleSubmit = () => {
    ...
    const submission = {
      ...
      content: JSON.stringify(isMultiStep ? values : { file: file?.name }),
      ...
      {isMultiStep && steps.length > 1 && (
        <div className="bg-card border border-card-border rounded-xl p-5">
          ...
          <div className="flex gap-2">
            {steps.map((step, i) => {
              const isDone = !!values[step.key]?.trim();
              return (
                <button
                  key={step.key}
        ...
        <div>
          ...
          <input
            ...
            className={`w-full bg-neutral border rounded-lg py-2 px-3 text-sm text-white placeholder-gray-600 focus:outline-none transition-colors ${
              errors.teamName ? 'border-red-500' : 'border-card-border focus:border-primary/50'
            }`}
          />
